  ...
  return (
    <>
      ...
      {/*
        ...
        A `<form>` inside a `<form>` is invalid HTML, and browsers resolve it by dropping the inner one —
        so a Remove button placed among the Save controls would have submitted the article editor instead.
        Below and separated is also the honest position for it: it is not one of the things you save.

        Gated on the current status rather than on `isPublished`, which means "has ever been published".
        A published article is refused by the action, and offering a button that can only say no is worse
        than not offering it — the status field it would point at is a few lines up.
      */}
      {article && article.status !== 'published' && (
        <div className="mt-8 border-t border-line pt-4">
          <RemoveControl
            ...
            label={article.title.sq || article.slug}
            noun="article"
