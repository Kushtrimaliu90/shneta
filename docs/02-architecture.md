...
_most_ scanners read is worse than none. The two alternatives were both worse — a QR image service is
...
matrix — the engine's inputs are fetched server-side and only the _result_ reaches the browser.
...
- **The emails** (`merchants/email.ts`, `merchants/partial-shipment-email.ts`) resolve a recipient across
  ...
  email-dispatch entries; the partial-shipment case additionally _sweeps_ orders, because the transition
