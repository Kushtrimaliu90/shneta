...
- **Unit (Vitest, `tests/`):** pure logic — `money.ts` (format sq/en, totals parity with RPC formulas, rounding cases), coupon math, free-shipping thresholds, `pickLocale`, slug utils, Zod schemas (valid/invalid fixtures), finder scoring, reading-time calc. Target: every exported function in `lib/` and every schema.
...
- **E2E (Playwright, `e2e/`):** desktop Chrome + mobile viewport (390×844). Journeys:
  ...
  10. Search typo-tolerance; compare page from 3 products; finder end-to-end.
